...
/**
 * Delete all offer documents tied to a product.
 * @param {string} productId - Product document id
 * @returns {Promise<void>}
 */
export async function deleteOffersForProduct(productId) {
  try {
    if (!productId) {
      return;
    }
    ...
    await Promise.all(snapshot.docs.map((offerDoc) => deleteDoc(offerDoc.ref)));
  } catch (error) {
    console.error('Error deleting offers for product:', error);
    throw error;
  }
}
