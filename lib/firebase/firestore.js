...
import { expandCompoundSizeRows } from '../utils/productSizes';
...
function computeNextInventory(productData, orderItem, direction) {
  ...
  const sizes = expandCompoundSizeRows(
    rawSizes
      .map((row) => ({
        size: String(row?.size || '').trim(),
        quantity: Number(row?.quantity ?? 0),
      }))
      .filter((row) => row.size && Number.isFinite(row.quantity) && row.quantity >= 0)
  );
